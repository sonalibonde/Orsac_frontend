 ...
 <div class="modal fade" id="myModal1" role="dialog">
   ...
   <div id="map" class="map"></div>
    ...
    //	var latt,longit,index=0;
    	...
    	try{
    	    $.ajax({
    	        url: "http://localhost:8080/indtrackwebservice/rest/CallService/getkey"
    	    }).then(function(data) {
    	       key = data.key;
    	    });
    	}catch(e){
...
//     	  }
    	  $scope.namesub = 'All';
    	  try{
// 		  alert('http://localhost:8080/indtrackwebservice/rest/CallService/getallvehiclesbycircle?circle='+circle);
    	  $http.get('http://localhost:8080/indtrackwebservice/rest/CallService/getallvehiclesbycircle?circle='+circle).success(
...
//   					alert('error: data = ',data);
    	  ...
    	  $http.get('http://localhost:8080/indtrackwebservice/rest/CallService/companydetails?companyid=30001&loginid=10001')
    	 ...
    	 // document.getElementById("tripinput").onchange = function(){
    		$scope.gettripdet = function(){ 
        	 ...
        	 document.getElementById('etpno').hidden = true;
	    	  ...
	    	  $http.get('http://localhost:8080/indtrackwebservice/rest/CallService/tripdet?vehicleno='+document.getElementById('tripinput').value+
    		...
    		var over = '<div id="overlay"></div>';
  		    ...
  		    clearInterval(interval);
    			...
    			$http.get('http://localhost:8080/indtrackwebservice/rest/CallService/gethistorytrip?vehicleno='+ vehicleno
...
//						alert("No data Found");
    	  ...
    	  $scope.allMarkers = function(){
    		  var over = '<div id="overlay"></div>';
    		    ...
    		    clearInterval(interval);
				clearInterval(intervall1);
    		  $http.get('http://localhost:8080/indtrackwebservice/rest/CallService/getallvehicles?ownersid='+ownersid).success(
    	  				function(data, status, headers,config) {
...
//     	  $http.get('http://localhost:8080/indtrackwebservice/rest/CallService/selectdistrict').success(
// 	  				function(data, status, headers,config) {
// 	  					$scope.distveh = data;
// 	  				}).error(function(data, status, headers,config) {
// 	  				});
    	  ...
    	  $scope.showDistrictVeh = function(loginid){
  		    ...
  		    clearInterval(interval);
				clearInterval(intervall1);
  		  $http.get('http://localhost:8080/indtrackwebservice/rest/CallService/getallvehicles?ownersid='+loginid).success(
  	  				function(data, status, headers,config) {
...
//   	  					alert('error: data = ',data);
    	  ...
    	  $scope.livefunctionTrail = function livefunctionTrail() {
    		  var over = '<div id="overlay"></div>';
				...
				$http.get('http://localhost:8080/indtrackwebservice/rest/CallService/getlivevehicles?vehicleno='+ document.getElementById("liveinput").value)
						.success(function(data, status,headers, config) {
									index1 = 0;
									removeFeatures();
									if(data.length>0){
...
// 								alert("No data Found");
    	  ...
    	  $scope.getDrawnFeature = function() {
				...
				if (route == "") {
					...
					//alert('http://localhost:8080/indtrackwebservice/rest/CallService/geofenceinsert?geofencename='+route+'&loginid='+ownersid+'&coord='+feature);
					$http.get('http://localhost:8080/indtrackwebservice/rest/CallService/geofenceinsert?geofencename='+route+'&loginid='+ownersid+'&coord='+feature)
					.success(function(data, status, headers, config) {
						$('#overlay').remove();
...
// 					alert('http://localhost:8080/indtrackwebservice/rest/CallService/routeinsert?routename='+route+
// 						  '&routetype='+ptype+'&loginid='+ownersid+'&coord='+feature);
					$http.get('http://localhost:8080/indtrackwebservice/rest/CallService/routeinsert?routename='+route+
							'&routetype='+ptype+'&loginid='+ownersid+'&coord='+feature)
					.success(function(data, status, headers, config) {
						$('#overlay').remove();
						alert("saved successfully");
...
// 						console.log('error: data = ', data);
    	  ...
    	  $scope.showFence = function(){
				...
				clearInterval(interval);
				clearInterval(intervall1);
				$http.get('http://localhost:8080/indtrackwebservice/rest/CallService/geofencedetails?companyid=30003&loginid='+ownersid+'')
				.success(function(data, status, headers, config) {
									try {
										if(data == ""){
											$('#overlay').remove();
...
// 									console.log('error: data = ',data);
    	  ...
    	  $scope.showRoute = function(){
  			var over = '<div id="overlay"></div>';
				...
				clearInterval(interval);
				clearInterval(intervall1);
				$http.get('http://localhost:8080/indtrackwebservice/rest/CallService/routedetails?companyid=1&loginid='+ownersid+'')
				.success(function(data, status, headers, config) {
											$scope.routename = data;
...
// 									console.log('error: data = ',data);
    	  ...
    	  $scope.process = function(){
    		  ...
    		  if(routename!=''){
        		  ...
        		  $http.get('http://localhost:8080/indtrackwebservice/rest/CallService/shelterdetails?companyid=30003&loginid='+ownersid+'&routename='+routename)
    				.success(function(data, status, headers, config) {
    									try {
    										if(data == ""){
    										} else{
...
//     		  }
    		  ...
    		  if(k==8){
    			  ...
    			  var response1 = $http.post('http://localhost:8080/indtrackwebservice/rest/CallService/shelterassigndelete?routeid='+routeid, 
    					  $scope.shelterassignlist);
					response1.success(function(data, status, headers, config) {
						 var response = $http.post('http://localhost:8080/indtrackwebservice/rest/CallService/shelterassign?routeid='+routeid, 
		    					  $scope.jsonArr);
    	  ...
    	  $scope.showShelter = function(){
    			var over = '<div id="overlay"></div>';
  				...
  				clearInterval(interval);
  				clearInterval(intervall1);
  				$http.get('http://localhost:8080/indtrackwebservice/rest/CallService/shelterdetails?companyid=30003&loginid='+ownersid+'')
  				.success(function(data, status, headers, config) {
  									try {
  										if(data == ""){
  											$('#overlay').remove();
...
//   											$scope.shelterassignlist = data;
				...
				$http.get('http://localhost:8080/indtrackwebservice/rest/CallService/poidetails?companyid=30003&loginid='+ownersid+'')
				.success(function(data, status, headers, config) {
									try {
										if(data == ""){
										} else{
...
// 											$scope.shelterassignlist = data;
    	  ...
    	  $scope.updatePoi = function(){
    		  ...
    		  $http.get('http://localhost:8080/indtrackwebservice/rest/CallService/poiupdate?loginid='+ownersid+'&id='+id+
...
//	 											$scope.shelterassignlist = data;
    	  ...
    	  $scope.deletePoi = function(){
    		  try{
	    		  var name1 = document.getElementById("updatepoiname");
	    		  var id = name1.options[name1.selectedIndex].value;
	    		  $http.get('http://localhost:8080/indtrackwebservice/rest/CallService/poidelete?loginid='+ownersid+'&id='+id)
					.success(function(data, status, headers, config) {
						$http.get('http://localhost:8080/indtrackwebservice/rest/CallService/poidetails?companyid=30003&loginid='+ownersid+'')
						.success(function(data, status, headers, config) {
											try {
